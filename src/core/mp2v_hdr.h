...
MP2V_INLINE bool write_slice_header(bitstream_writer_c* m_bs, slice_t& slice, sequence_header_t& sh, sequence_scalable_extension_t* sequence_scalable_extension) {
    ...
    if (sequence_scalable_extension && sequence_scalable_extension->scalable_mode == scalable_mode_data_partitioning)
        m_bs->write_bits(slice.priority_breakpoint, 7);
    m_bs->write_bits(slice.quantiser_scale_code, 5);
    if (slice.slice_extension_flag == 1) {
        m_bs->write_bits(slice.slice_extension_flag, 1);
        m_bs->write_bits(slice.intra_slice, 1);
        m_bs->write_bits(slice.slice_picture_id_enable, 1);
        m_bs->write_bits(slice.slice_picture_id, 6);
    }
    m_bs->zero_bit();
    return true;
}
