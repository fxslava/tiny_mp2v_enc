...
void decode_block_template(pixel_t* plane, uint32_t stride, int16_t QFS[64], uint16_t W[64], uint16_t W_i[64], uint8_t quantizer_scale, int intra_dc_prec) {
    int16_t QF[64];
    int16_t F[64];
...
void decode_macroblock_template(
    ...
    if (mb_intra) {
        ...
        if (mb_data.pattern_code[1])
            decode_block_template<uint8_t, alt_scan, true>(yuv_planes[0] + 16, stride, mb_data.QFS[1], W[0], W[1], quantizer_scale, intra_dc_prec);
        if (mb_data.pattern_code[2])
            decode_block_template<uint8_t, alt_scan, true>(yuv_planes[0] + 16 * stride, stride, mb_data.QFS[2], W[0], W[1], quantizer_scale, intra_dc_prec);
        if (mb_data.pattern_code[3])
            decode_block_template<uint8_t, alt_scan, false>(yuv_planes[0] + 16 * (stride + 1), stride, mb_data.QFS[3], W[0], W[1], quantizer_scale, intra_dc_prec);
        ...
        if (chroma_format == 2) {
            ...
            if (mb_data.pattern_code[5])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[1] + 16, chroma_stride, mb_data.QFS[5], W[2], W[3], quantizer_scale, intra_dc_prec);
            ...
            if (mb_data.pattern_code[7])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[2] + 16, chroma_stride, mb_data.QFS[7], W[2], W[3], quantizer_scale, intra_dc_prec);
        }
        if (chroma_format == 3) {
            if (mb_data.pattern_code[4])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[1], stride, mb_data.QFS[4], W[2], W[3], quantizer_scale, intra_dc_prec);
            if (mb_data.pattern_code[5])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[1] + 16, stride, mb_data.QFS[5], W[2], W[3], quantizer_scale, intra_dc_prec);
            if (mb_data.pattern_code[6])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[1] + 16 * stride, stride, mb_data.QFS[6], W[2], W[3], quantizer_scale, intra_dc_prec);
            if (mb_data.pattern_code[7])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[1] + 16 * (stride + 1), stride, mb_data.QFS[7], W[2], W[3], quantizer_scale, intra_dc_prec);
            ...
            if (mb_data.pattern_code[9])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[2] + 16, stride, mb_data.QFS[9], W[2], W[3], quantizer_scale, intra_dc_prec);
            if (mb_data.pattern_code[10])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[2] + 16 * stride, stride, mb_data.QFS[10], W[2], W[3], quantizer_scale, intra_dc_prec);
            if (mb_data.pattern_code[11])
                decode_block_template<uint8_t, alt_scan, true>(yuv_planes[2] + 16 * (stride + 1), stride, mb_data.QFS[11], W[2], W[3], quantizer_scale, intra_dc_prec);
        }
    }
}
